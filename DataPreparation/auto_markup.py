...
def auto_markup(filename: str):
    ...
    def set_left_hand():
        ...
        for i in left_interval:
            set_class(start=i[0]-cf.margin_for_markup, stop=i[1], classname=1)
    ...
    def set_right_hand():
        ...
        for i in left_interval:
            set_class(start=i[0]-cf.margin_for_markup, stop=i[1], classname=2)

    set_left_hand()
    set_right_hand()
    data = data.drop(['N', 'right', 'left', 'time'], axis=1)
    data.to_csv("../converted_data/markup_class{0}.csv".format(num), index=False, header=False)
    return "../converted_data/markup_class{0}.csv".format(num)